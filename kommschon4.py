...
# --- PAGE CONFIG ---
st.set_page_config(page_title="Uni-Planer", page_icon="📅", layout="centered")
...
def clean_title_logic(course_id, raw_title):
    """
    Cleaning Rules:
    - If ID is 'X.Y' (2 digits, 1 dot) -> Cut off at dash (General Module Name).
    - If ID is 'X.Y.Z' (3 digits, 2 dots) -> Keep full title (Specific Group/Seminar).
    """
    ...
    # For "1.3.1" or deeper, return full title
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader("Vorlesungsverzeichnis hochladen", type="pdf")
    ...
    sem_start = st.date_input("Beginn des Semesters", key='sem_start_date')
    sem_end = st.date_input("Ende des Semesters", key='sem_end_date')
...
# 3. SEARCH & SELECT
st.subheader("Module und Vorlesungen auswählen")
query = st.text_input("Modulnummern eingeben (z.B. '1.3, 4.2'), oder einzelne Vorlesungen (z.B. '1.3.3')", placeholder="Mehrere Einträge mit Kommata trennen...")
...
if query:
    ...
    if not matched_events:
        ...
        st.divider()
        st.write("""
            Aus den eingegebenen Modulen sind alle Vorlesungen standardmäßig als belegt ausgewählt. \n
            Belegte Seminare und Gruppen müssen von dir selber ausgewählt werden. Du findest deine auf http://seminarwahlen.kh-freiburg.de/. \n
            Du kannst ebenfalls einzelne Lehrveranstaltungen wieder abwählen. \n\n\n\n
        """)
        ...
        for eid in sorted_ids:
            ...
            # e.g. "1.3.2 Seminar A", "1.3.2 Seminar B" -> Show Multiselect or List of Checkboxes
                ...
                st.markdown(f"**{eid} - Bitte Gruppe oder Seminar wählen:**")
                for t in titles:
                    c1, c2 = st.columns([0.1, 0.9])
...
if final_events_to_process:
    st.divider()
    st.subheader("Vorschau der ausgewählten Veranstaltungen")
